...
class MLogix_Wedding_Model_Wedding extends Mage_Core_Model_Abstract
{
    public function _construct()
    {
    	...
    	$path = Mage::getBaseDir('media') . DS . 'wedding';
    	$thumbpath = Mage::getBaseDir('media') . DS . 'wedding' . DS . 'thumbs';
    ...
    {
    	return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'wedding/';    	
    }
    ...
    {
    	$root = Mage::getModel('wedding/wedding')->load(0);
    ...
    public function move($newParentId, $afterId, $categoryId=0)
    {
        ...
        try {
			...
			$tableName = $this->getResource()->getTable('wedding/wedding');

			$write = Mage::getSingleton('core/resource')->getConnection('core_write');
			
			$write->raw_query("update $tableName set after_id = $prevAfterId where after_id = $categoryId");			
			$write->raw_query("update $tableName set after_id = $categoryId where after_id = $afterId and parent_id = $newParentId and wedding_id != $categoryId");			
...
public function createthumb($name,$filename,$new_w,$new_h,$forcepng=false){
	...
	*/
    ...
    public function makeThumbnail($width, $height, $refresh=false)
    {
    	...
    	$thumbpath = Mage::getBaseDir('media') . DS . 'wedding' . DS . 'thumbs' . DS;
    ...
    public function getBreadcrumbPath()
    {
		...
		$read = $resource->getReadConnection();		
   		...
   		while($parent != 0)
   		{
   			...
   			if($count++ > 100) { echo 'Error in Model/Wedding'; die(); }
   			
	   		$sql = "SELECT wedding_id, parent_id FROM ".$table." where wedding_id = ?";
	   		//echo $sql;
	   		//die();
			$stmt = $read->query($sql, $parent);
			
			if ($row = $stmt->fetch()) {
			  $path[] = Mage::getModel('wedding/wedding')->load($row['wedding_id']);
    ...
    public function hasChildren()
    {
		...
		$read = $resource->getReadConnection();		
   		...
   		$sql = "SELECT wedding_id, parent_id FROM ".$table." where parent_id = ?";
   		//echo $sql;
   		//die();
