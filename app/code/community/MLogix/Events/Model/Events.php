...
class MLogix_Events_Model_Events extends Mage_Core_Model_Abstract
{
    public function _construct()
    {
    	...
    	$thumbpath = Mage::getBaseDir('media') . DS . 'events' . DS . 'thumbs';
    ...
    {
    	return Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA).'events/';    	
    }
    ...
    {
    	$root = Mage::getModel('events/events')->load(0);
    ...
    public function move($newParentId, $afterId, $categoryId=0)
    {
        ...
        try {
			...
			$tableName = $this->getResource()->getTable('events/events');

			$write = Mage::getSingleton('core/resource')->getConnection('core_write');
			
			$write->raw_query("update $tableName set after_id = $prevAfterId where after_id = $categoryId");			
			$write->raw_query("update $tableName set after_id = $categoryId where after_id = $afterId and parent_id = $newParentId and events_id != $categoryId");			
...
public function createthumb($name,$filename,$new_w,$new_h,$forcepng=false){
	...
	*/
    ...
    public function makeThumbnail($width, $height, $refresh=false)
    {
    	...
    	$thumbpath = Mage::getBaseDir('media') . DS . 'events' . DS . 'thumbs' . DS;
    ...
    public function getBreadcrumbPath()
    {
		...
		$read = $resource->getReadConnection();		
   		...
   		while($parent != 0)
   		{
   			...
   			if($count++ > 100) { echo 'Error in Model/Events'; die(); }
   			
	   		$sql = "SELECT events_id, parent_id FROM ".$table." where events_id = ?";
	   		//echo $sql;
	   		//die();
			$stmt = $read->query($sql, $parent);
			
			if ($row = $stmt->fetch()) {
			  $path[] = Mage::getModel('events/events')->load($row['events_id']);
    ...
    public function hasChildren()
    {
		...
		$read = $resource->getReadConnection();		
   		...
   		$sql = "SELECT events_id, parent_id FROM ".$table." where parent_id = ?";
   		//echo $sql;
   		//die();
