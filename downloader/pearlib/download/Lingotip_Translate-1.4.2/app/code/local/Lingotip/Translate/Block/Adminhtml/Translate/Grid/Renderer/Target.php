...
if(isset($isForSelectTT) && $isForSelectTT != "")
{
	//$JSArray .= '['.'js'.$row->getId().$target.']'.'['.$translatedText.']';
	$toolTip1 = "Select to insert this translation directly into Magento";
	$result[] .=  '<div><div style="'. $float. $color .'" alt="'.$toolTip.'" title="'.  ucfirst($toolTip) .'" >'.$target.'</div>';
	$result[] .= '<div style="'. $border.'"><button style="margin-bottom:5px;" onclick=translateTextOnGrid("js'.$row->getId().$target.'","'.$textBoxId.'","'.$status.'") alt="'.$toolTip1.'" title="'.  ucfirst($toolTip1) .'" >'."Select".'</button><br>';
	$result[] .= '</div></div>';
	...
	//return $res = implode(",",$result);
		...
		//$JSArray .= '['.'js'.$row->getId().$target.']'.'['.$translatedText.']';
		$result[] = '<span onclick=translateTextOnGrid("transdata'.$row->getId().$target.'","'.$textBoxId.'","'.$status.'") alt="'.$toolTip.'" title="'.  ucfirst($toolTip) .'" style="'.  $color .'">'.$target.'</span>';
//		'<div style="display:none;" id="transdata'.$row->getId().$target.'">'.htmlentities($translatedText).'</div>';
		//$result[] ' ';
	}		
