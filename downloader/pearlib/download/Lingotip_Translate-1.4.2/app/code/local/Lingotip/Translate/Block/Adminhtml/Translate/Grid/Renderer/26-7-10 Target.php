...
if(isset($isForSelectTT) && $isForSelectTT != "")
{
	...
	$result[] .= '<div style="'. $border.'"><button style="margin-bottom:5px;" onclick=translateTextOnGrid("transdata'.$row->getId().$target.'","'.$textBoxId.'","'.$status.'","'.$row->getId().'","'.$target.'") alt="'.$toolTip.'" title="'.  ucfirst($toolTip) .'" >'."Select".'</button><br>';
	$result[] .= '</div></div>';
