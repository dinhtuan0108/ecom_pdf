...
{
	$result[] = '<table celpadding="0" cellspacing="0">';
}	
...
if(isset($isForSelectTT) && $isForSelectTT != "")
{
	$result[] .=  '<tr><td style="'. $border. $color .'" alt="'.$toolTip.'" title="'.  ucfirst($toolTip) .'" >'.$target.'</td>';
	$result[] .= '<td style="'. $border.'"><button style="margin-bottom:5px;" onclick=translateTextOnGrid("transdata'.$row->getId().$target.'","'.$textBoxId.'","'.$status.'") alt="'.$toolTip.'" title="'.  ucfirst($toolTip) .'" >'."Select".'</button><br>';
	$result[] .= '</td></tr>';
	//return $res = implode(",",$result);
}
else{
		$result[] = '<span onclick=translateTextOnGrid("transdata'.$row->getId().$target.'","'.$textBoxId.'","'.$status.'") alt="'.$toolTip.'" title="'.  ucfirst($toolTip) .'" style="'.  $color .'">'.$target.'</span>'. '<div style="display:none;" id="transdata'.$row->getId().$target.'">'.$translatedText.'</div>';
	}		
		
		}
	if(isset($isForSelectTT) && $isForSelectTT != "")
	{
		$result[] = '</table>';return $res = implode("",$result);
	}		
		return $res = implode(",",$result);
	}	
